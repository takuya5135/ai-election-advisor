...
export async function findElections(residence: string) {
    try {
        ...
        const researchPrompt = `
    現在の日時は ${today} です。
    居住地「${residence}」のユーザーに関係する選挙を調査してください。
    
    【最優先事項】
    2026年初頭（1月〜2月頃）に実施が予想される**「第51回衆議院議員総選挙（衆院選）」**および**「参議院議員通常選挙（参院選）」**の最新情報を必ず含めてください。
    これらは国政選挙として居住地にかかわらず全ユーザーに関係するため、確実に見つけ出してください。
    
    【調査範囲】
    1. **国政選挙**: 第51回衆議院議員総選挙、参議院議員通常選挙の公示・投開票日。
    2. **地方選挙**: ${residence}に関連する知事選、市長選、議会選挙。
    3. **日程の確定度**: 複数の説がある場合は、最新の報道や選管発表を基に最も可能性の高い日程を特定してください。
    `;
