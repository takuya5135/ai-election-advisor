...
const getBasePrompt = (electionName: string, userProfile: any) => {
    ...
    1. **形式**: 全ての質問は、ユーザーが「共感する」「共感しない」で明確に答えられる**「一つの肯定的な主張」または「具体的な提案」の形**で作成してください。
       - **禁止事項**: 「〜すべきか、それとも〜すべきか？」といった**二者択一形式や、「〜どう思いますか？」という疑問形の質問は絶対に避けてください**。これらは「共感する/しない」で答えるのには不適切です。
       - **必須形式**: 必ず「〜すべきだ。」「〜という考え方は正しい。」「〜に賛成だ。」という**言い切りの形**にしてください。
...
export async function translateQuestionsForAge(questions: any[], userProfile: any) {
    try {
        ...
        if (userProfile.age === "elementary") {
            ...
            【ターゲット: 小学校高学年 (10-12歳)】
            - **役割**: 社会科の授業のような「知的な先生」。
            - **文体**: 幼稚な「〜だよ」言葉ではなく、「〜です・ます」を基本としますが、**絶対に文末を「〜でしょうか？」のような疑問形にしないでください**。「〜だと思います」「〜したほうがいいです」のように、自分の意見として言い切ってください。
            ...
            - **変換方針**: 
              ...
              3. **重要: 質問文にしないこと**: ユーザーが「共感する」ボタンを押せるように、「〜するべきだと思います」や「〜のほうがいいです」という形式を崩さないでください。
            `;
        } else if (userProfile.age === "middle_high") {
            agePrompt = `
            【ターゲット: 中学生・高校生 (13-18歳)】
            - **役割**: 頼れる先輩や兄貴分。
            - **文体**: 教科書のような堅苦しさを排除し、SNSやYouTubeで見るような「自分たちに向けられた言葉」と感じるフランクだが知的なトーン。ただし、**疑問形（〜かな？〜どう思う？）は禁止**です。「〜だと思う」「〜のほうが良くない？」ではなく「〜のほうが良い」と言い切ってください。
            - **変換方針**:
        ...
        1. **JSON構造の維持**: 配列の順序やオブジェクトのキー（id, category, questionTypeなど）は**絶対に変更しないでください**。書き換えるのは以下のフィールドのみです：
           ...
           - analysis.impact (各項目)
        2. **意味と形式の維持**: 
           - 質問の意図や政策の核心が変わらないようにしてください。
           - **questionType="standard"の場合**: 元の質問が「〜すべきだ」という主張型であるため、**翻訳後も必ず「〜すべきだ」「〜だと思います」という主張型（言い切り）を維持してください**。「〜どう思いますか？」という疑問文には絶対にしないでください。
